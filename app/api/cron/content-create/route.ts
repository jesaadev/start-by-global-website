...
// actúa y cuántos artículos genera se configuran desde el admin (settings.ai.schedule).
...
export async function GET(request: Request) {
  ...
  if (!force && !schedule.createDays.includes(day)) {
    return NextResponse.json({ ok: true, skipped: `no es día programado (hoy=${day}, días=[${schedule.createDays.join(",")}])` })
  }
  try {
    const result = await runCreateRoutine(Math.max(1, schedule.createCount || 1), locale)
